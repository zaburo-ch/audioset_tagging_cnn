...
from panns.pytorch.evaluate import Evaluator
from panns.pytorch.losses import get_loss_func
from panns.pytorch.pytorch_utils import count_parameters, do_mixup, move_data_to_device
from panns.utils import config
from panns.utils.data_generator import (
...
from panns.utils.utilities import Mixup, StatisticsContainer, create_folder, create_logging, get_filename
...
def train(args):
    ...
    train_indexes_hdf5_path = os.path.join(workspace, "hdf5s", "indexes", "{}.h5".format(data_type))

    eval_bal_indexes_hdf5_path = os.path.join(workspace, "hdf5s", "indexes", "balanced_train.h5")
    ...
    eval_bal_loader = torch.utils.data.DataLoader(
        dataset=dataset, batch_sampler=eval_bal_sampler, collate_fn=collate_fn, num_workers=num_workers, pin_memory=True
    )
    ...
    for batch_data_dict in train_loader:
        ...
        if (iteration % 2000 == 0 and iteration > resume_iteration) or (iteration == 0):
            ...
            logging.info("Validate bal mAP: {:.3f}".format(np.mean(bal_statistics["average_precision"])))

            logging.info("Validate test mAP: {:.3f}".format(np.mean(test_statistics["average_precision"])))
