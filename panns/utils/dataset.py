...
from panns.utils.utilities import (
    create_folder,
    create_logging,
    float32_to_int16,
    get_filename,
...
def split_unbalanced_csv_to_partial_csvs(args):
    ...
    for r in range(files_num):
        ...
        out_csv_path = os.path.join(unbalanced_partial_csvs_dir, "unbalanced_train_segments_part{:02d}.csv".format(r))
...
def download_wavs(args):
    ...
    for (n, line) in enumerate(lines):
        ...
        logging.info("{} {} start_time: {:.1f}, end_time: {:.1f}".format(n, audio_id, start_time, end_time))
        ...
        if len(video_paths) > 0:
            ...
            os.system(
                "ffmpeg -loglevel panic -i {} -ac 1 -ar 32000 -ss {} -t 00:00:{} {} ".format(
                    video_path, str(datetime.timedelta(seconds=start_time)), duration, audio_path
                )
            )
...
if __name__ == "__main__":
    ...
    parser_download_wavs.add_argument(
        "--csv_path", type=str, required=True, help="Path of csv file containing audio info to be downloaded."
    )
    parser_download_wavs.add_argument(
        "--audios_dir", type=str, required=True, help="Directory to save out downloaded audio."
    )
    parser_download_wavs.add_argument(
        "--mini_data", action="store_true", default=True, help="Set true to only download 10 audios for debugging."
    )
    ...
    parser_pack_wavs.add_argument(
        "--waveforms_hdf5_path", type=str, required=True, help="Path to save out packed hdf5."
    )
    parser_pack_wavs.add_argument(
        "--mini_data", action="store_true", default=False, help="Set true to only download 10 audios for debugging."
    )
