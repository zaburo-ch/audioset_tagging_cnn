...
from panns.utils.utilities import create_folder, get_sub_filepaths
...
def combine_full_indexes(args):
    ...
    paths = [path for path in paths if ("train" in path and "full_train" not in path and "mini" not in path)]
    ...
    with h5py.File(full_indexes_hdf5_path, "w") as full_hf:
        ...
        full_hf.create_dataset(name="target", shape=(0, classes_num), maxshape=(None, classes_num), dtype=np.bool)
...
if __name__ == "__main__":
    ...
    parser_create_indexes.add_argument(
        "--waveforms_hdf5_path", type=str, required=True, help="Path of packed waveforms hdf5."
    )
    parser_create_indexes.add_argument(
        "--indexes_hdf5_path", type=str, required=True, help="Path to write out indexes hdf5."
    )

    parser_combine_full_indexes = subparsers.add_parser("combine_full_indexes")
    parser_combine_full_indexes.add_argument(
        "--indexes_hdf5s_dir", type=str, required=True, help="Directory containing indexes hdf5s to be combined."
    )
    parser_combine_full_indexes.add_argument(
        "--full_indexes_hdf5_path", type=str, required=True, help="Path to write out full indexes hdf5 file."
    )
